...
#[cfg(mpio)]
mod mpi {
    use hdf5::{Dataset, FileBuilder, Result, Selection};
    use mpi_sys::{
        MPI_Comm_rank, MPI_Comm_size, MPI_Finalize, MPI_Init, MPI_Initialized, RSMPI_COMM_WORLD,
    };
    use ndarray::{array, s, Array, Array1, ArrayView};
    ...
    where
        ...
        D: ndarray::Dimension,
        S: TryInto<Selection>,
        S::Error: Into<hdf5::Error>,
    {
        dset.write_slice(data.into(), selection.try_into().map_err(|err| err.into())?)
    }

    fn read<'a, S, D>(dset: &Dataset, selection: S) -> Result<Array<f64, D>>
    where
        D: ndarray::Dimension,
        S: TryInto<Selection>,
        S::Error: Into<hdf5::Error>,
